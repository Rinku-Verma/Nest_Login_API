...
export class UsersService {
  ...
  constructor( @InjectRepository(User) private userRepo: Repository<User>, private jwt: JwtService){}
...
//----->>>>     LOGIN USER SIGNIN
  async login(loginUser: any){
    try{
      ...
      if(isPresent){
        const flag = await bcrypt.compare(loginUser.password ,isPresent.password);
        if(flag){
...
//----->>>> UPDATE USER DATA
  async update(id: any, updateUser: any) {
    try{
      const isPresent = await this.userRepo.findOneBy({id: id});
      if(isPresent){
        const flag = await bcrypt.compare(updateUser.password, isPresent.password);
        if(flag){
...
//----->>>>>      LOGOUT USER  
  ...
  async refresh(body: RefreshDTO){
    ...
    try{
      // console.log('jj')
      const id = this.jwt.verify(body.refreshToken, {secret: process.env.REFRESH_TOKEN});
      const isPresent = await this.userRepo.findOneBy({id: id.id});
      // console.log(isPresent)
      if( isPresent ){
        // console.log('kk');
        const flag = await bcrypt.compare(body.refreshToken ,isPresent.refresh_token);
        if(flag){
          const payload = {
            id: isPresent.id,
